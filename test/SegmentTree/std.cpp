...
namespace SegTree {
    ...
    void update(int s, int t, int d, int k, int l, int r) {
        ...
        pushdown(k, l, r);
        int mid = l + (r - l >> 1);
        if (s <= mid) update(s, t, d, lson(k), l, mid);
        if (t > mid) update(s, t, d, rson(k), mid + 1, r);
        pushup(k);
    }
    ...
    int query(int s, int t, int k, int l, int r) {
        ...
        if (s <= mid) sum += query(s, t, lson(k), l, mid);
        if (t > mid) sum += query(s, t, rson(k), mid + 1, r);
        return sum;
    }
};
...
signed main( signed argc, char *argv[] ) {
    ...
    SegTree::build(1, 1, n);
    while (m--) {
        int op, l, r, d; fin >> op >> l >> r;
        if (op == 1) {
            fin >> d;
            SegTree::update(l, r, d, 1, 1, n);
        } else fout << SegTree::query(l, r, 1, 1, n) << endl;
    }
