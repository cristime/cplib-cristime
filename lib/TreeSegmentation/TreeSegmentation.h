...
namespace SegTree
{
    ...
    {
        if ( !tag[ k ] ) return;    // If tag is empty, doing the following staff is useless.
    ...
    void build( T *origin, int *rnk, const int k, const int l, const int r )      // Build tree from an array
    {
        // If it's a leaf node
        if ( l == r )
        {
...
{   
    ...
    void dfs1( int u, int pre )
    {
        ...
        for ( auto &v : edge[ u ] )
        {
            ...
            if ( sz[ v ] > sz[ son[ u ] ] ) 
                son[ u ] = v;           // Update son with the biggest weight
        }
    }
    ...
    {
        SegTree::update( dfn[ u ], dfn[ u ] + sz[ u ] - 1, delta, 1, 1, n );               // Update the subtree
    }

    // Query the sum of the subtree
    T QuerySubTree( int u )
    {
        return SegTree::query( dfn[ u ], dfn[ u ] + sz[ u ] - 1, 1, 1, n );                 // Query the subtree
    }
